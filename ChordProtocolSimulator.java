...
public class ChordProtocolSimulator {
    ...
    // each key is indexed using consistent hashing. eg:- the tuple (key 1, 123)
    // represents a key with a name
    // 'key 1' and the index value of 123. Each key is assumed to have a unique
    // name.
     ...
     *
     * First keys are generated randomly. Each key is assigned a key name and it's
     * store in the keyIndexes. Then key
     * indexes are added to the node.
     *
     * For each key index:
     * 1) find the node that should be responsible for the key (the index of the
     * node should be greater than the
     * index of key and the key index should be close to node index in the
     * consistent hash ring)
     * 2) add the key index to the node (by calling the peer.add_data() from the
     * network)
     */
    public void assignKeys() {
        ...
        for (Map.Entry<String, Integer> entry : keyIndexes.entrySet()) {
            ...
            // System.out.println("key name: "+name +"\t key_index : "+key_index+"\t
            // peer_name : "+peer_name);
    ...
    /**
     * This method generates 'KeyCount' number of keys. each key has name "key"+i
     * i=1...KeyCount
     * For each key:
     ...
     * It retrieves the set of nodes from the network and for each node calculates
     * the index of the node using
     * consistent hashing. Then it compares the index of the key with index of all
     * the nodes. It choose a node
     * that has an index that would be next to the key index in the consistent hash
     * ring.
     *
    ...
    public String findPeer(int key_index) {
        ...
        // if it is larger then the key should be placed before the lowest node index
        // (at the start of the ring)
        if (key_index > sortedIndexes.get(sortedIndexes.size() - 1)) {
            peerIndex = sortedIndexes.get(0);
        } else {
            // if the key index is not larger than the biggest node index, then choose the
            // node that has index
            // bigger than the key index. loop through the nodes in the ascending order of
            // the node indexes
        ...
        // if no node is found then return null. Note: If everything works well then
     ...
     * This method uses a hack that the network setup uses the names Peer 1, Peer
     * 2,... etc. It would be nice if this is
     * automatic. like fetching all the nodes from the network and compare the names
     * with the given node (based on
    ...
    /**
     * This method prints the network. The network consists of set of nodes. It
     * prints different information contained
    ...
    /**
     * This method prints the ring overlay that chord protocol uses. It follows
     * circular linked list algorithm to travel
     * the network. It starts with head and goes through successors. I am assuming
     * that each peer stores only one
     * neighbor but i think the chord protocol doesn't make this restriction. but
     * still even if the node has multiple
    ...
    /**
     * This method tests the functioning of the lookup. This is a simple evaluation.
     * For each key index it calls the
     * lookup from the chord protocol and returns the node index. It then compares
     * the node index with the correct node
    ...
    /**
     * This method compares whether the node actually stores the given key index or
     * not
     * It retrieves the data items stored at the particular node and compares
     * whether the key index is stored or not
     * 
    ...
    public void start() {
        ...
        System.out.println("Starting simulation (CHORD)");
        // builds the protocol
        buildProtocol();
        ...
        // Should print the result to the terminal and an output file using
        // result.toString()
        // also calculate average hop count which is also specified in the result
        // Look up all the keys defined in the `keyIndexes` map.
        List<Integer> hopCounts = new ArrayList<>();
        for (Map.Entry<String, Integer> entry : keyIndexes.entrySet()) {
            String keyName = entry.getKey();
            int keyIndex = entry.getValue();

            // Perform lookup for each key and collect responses.
            LookUpResponse response = protocol.lookUp(keyIndex);
            if (response != null) {
                System.out.println(keyName + ":" + keyIndex + " " + response.toString());
                hopCounts.add(response.peers_looked_up.size()); // Collect hop counts to calculate the average later.
            }
        }

        // Calculate the average number of hops required during the lookups.
        double averageHopCount = hopCounts.stream().mapToInt(Integer::intValue).average().orElse(0.0);
        System.out.println("Average Hop Count: " + averageHopCount);
    }
